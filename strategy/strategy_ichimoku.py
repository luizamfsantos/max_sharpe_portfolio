...
def calculate_conversion_line(data:pd.DataFrame,rolling_periods:int=9) -> pd.Series:
    """
...
def calculate_baseline(data:pd.DataFrame,rolling_periods:int=26) -> pd.Series:
    """
...
def calculate_leading_span_A(
    ...
    if 'conversion_line' not in data.columns:
        data['conversion_line'] = calculate_conversion_line(data,conversion_rolling_periods)
...
def calculate_lagging_span(data:pd.DataFrame, lagging_periods:int=30) -> pd.Series:
    """ 
...
def calculate_cloud(
    ...
    if 'leading_span_A' not in data.columns:
        data['leading_span_A'] = calculate_leading_span_A(data,baseline_rolling_periods,conversion_rolling_periods,future_periods)
    if 'leading_span_B' not in data.columns:
        data['leading_span_B'] = calculate_leading_span_B(data,baseline_rolling_periods,future_periods)
